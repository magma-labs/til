...
namespace :db do
  ...
  def verify_exec(command)
    system(command)
    raise 'Command failed' unless $CHILD_STATUS.exitstatus.zero?
  end

  file dump_file do
    verify_exec "heroku pg:backups capture --app #{ENV.fetch('production_app_name')}"
    verify_exec "curl -o #{dump_file} `heroku pg:backups public-url -a #{ENV.fetch('production_app_name')}`"
  end
