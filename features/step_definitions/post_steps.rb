...
Given 'a channel with a more info partial' do
  @channel = FactoryBot.create(:ruby_channel)
end
...
Given 'a draft post exists in the ruby channel' do
  @post = FactoryBot.create(:post, :draft, channel: FactoryBot.create(:ruby_channel))
end

Given 'a post exists in the ruby channel' do
  @post = FactoryBot.create(:post, channel: FactoryBot.create(:ruby_channel))
end
...
Given(/^(\d+) drafts* exists* by another developer$/) do |num|
  FactoryBot.create_list(:post, num.to_i, :draft)
end
...
Given 'there exist posts for today, yesterday, and last week' do
  ...
  @rails_post   = FactoryBot.create(:post, :for_last_week, developer: rails_dev,
                                                           body: 'I learned about Rails')
  @ember_post   = FactoryBot.create(:post, :for_yesterday, developer: ember_dev,
                                                           body: 'I learned about Ember')
  @karate_post  = FactoryBot.create(:post, :for_today, developer: karate_dev,
                                                       body: 'I learned about Karate')
end
...
Then 'I see the second most recent post' do
  expect(page).to have_current_path(post_path(@older_post), ignore_query: true)
end
...
Then 'I see the least recent post' do
  expect(page).to have_current_path(post_path(@oldest_post), ignore_query: true)
end
...
Given 'I visit the author page of an author with a Twitter handle' do
  developer = FactoryBot.create(:developer, twitter_handle: 'sqlfan')
...
Then 'I see the show page for that post' do
  expect(page).to have_current_path "/posts/#{@post.slug}-web-development", ignore_query: true
